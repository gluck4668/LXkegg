...
go_BP <- go_BP_all[c(1:n_BP),]%>% na.omit()
go_CC <- go_CC_all[c(1:n_CC),]%>% na.omit()
go_MF <- go_MF_all[c(1:n_MF),]%>% na.omit()
...
#color_text <- c(rep("#20b2aa",nrow(go_BP)),rep("#6666cc",nrow(go_MF)))
#textcolor_theme <- theme(axis.text.x = element_text(colour = color_text))
...
if(n_BP>0)
  {bb_text <- "Biological process(BP)"
  bb_color="#20b2aa"} else
    {bb_text <-""
    bb_color=""}

if(n_CC>0)
  {cc_text <- "Cellular component (CC)"
   cc_color="#d2691e"}else
     {cc_text <- ""
     cc_color=""}

if(n_MF>0)
  {mf_text <- "Molecular function (MF)"
  mf_color="#6666cc"}else
    {mf_text <-""
    mf_color=""}

val_color <- c(bb_color,cc_color,mf_color) %>% .[.!=""]

legend_text <- c(bb_text,cc_text,mf_text) %>% .[.!=""]
...
GO_enrich01 <- GO_plot+ geom_bar(position = "dodge",stat = "identity",width = 0.8)+
  ...
  scale_fill_manual(values = val_color,label=legend_text)+
...
write.xlsx(go_BP,GO_BP_ex)
  ...
  print("There is no BP in the GO analysis")
#-----------------------CC analysis----------------------------------#
...
write.xlsx(go_CC,GO_CC_ex)
  ...
  print("There is no CC in the GO analysis")
#------------------------------MF analysis-------------------------------#
...
write.xlsx(go_MF,GO_MF_ex)
  ...
  print("There is no MF in the GO analysis")
