  ...
  #-----------------up and down-regulation--------------
    ...
    if(length(kegg_up)>0 & length(kegg_down)>0){
      ...
      if(length(meta_up_n)>0){
        ...
        kegg_up_10 <- kegg_up_10[c(1:10),]
      } else {
        kegg_up_10 <- dplyr::arrange(kegg_up,pvalue) 
        kegg_up_10 <- kegg_up_10[c(1:10),]
      }
      ...
      if(length(meta_down_n)>0){
        ...
        kegg_down_10 <- kegg_down_10[c(1:10),]
        } else {
           kegg_down_10 <- dplyr::arrange(kegg_down,pvalue) 
           kegg_down_10 <- kegg_down_10[c(1:10),]
           }
      ...
      kegg_10 <- rbind(kegg_up_10,kegg_down_10)
      ...
      kegg_10$log10p_down <- purrr::map(c(1:nrow(kegg_10)),~{if(kegg_10[.x,ncol(kegg_10)-1]=="down")
                                                               kegg_10[.x,ncol(kegg_10)+1]=round(kegg_10[.x,ncol(kegg_10)-2],2) else
                                                               kegg_10[.x,ncol(kegg_10)+1]=""  
                                                             } )
      ...
      p0 <- ggplot(kegg_10,aes(x =reorder(Description,log10pvalue),y =log10pvalue,fill=Type))+
        ...
        geom_segment(aes(y=0,yend=0,x=0,xend=20.8)) # 加一条坚线
      ...
      #添加箭头
      arrow_down <- geom_segment(aes(y=-0.2,yend=ymin, x=21,xend=21),
                                 arrow=arrow(length = unit(0.2,"cm"),type="closed"),
                                 linewidth=0.5
                                 )
      
      
      arrow_up <- geom_segment(aes(y=0.2,yend=ymax, x=21,xend=21),
                               arrow=arrow(length = unit(0.2,"cm"),type="closed"),
                               linewidth=0.5
                              )
      ...
      p1 <- p0+my_theme+
        ...
        arrow_down + 
        arrow_up+ 
        annotate("text",x=21.6,y=ymin+1,label="Down",)+
        annotate("text",x=21.6,y=ymax-1,label="Up")+
        col_value_up+col_value_down+
        ylab("-log10(pvalue)")+
      ...
      path_10 <- paste(species,"KEGG enriched pathways (up and down).png")
      path_10 <-paste0(dir.file,"/",path_10)
      ggsave(path_10,p1,width = 1200,height =1000,units = "px",dpi = 150)
