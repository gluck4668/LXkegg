...
LXkegg <- function(gene_file,group1,group2,species){
  ...
  {spec_txt <- paste("The species that you writed was",species,", which was not inluced in the species of this R package (human, mouse, or rat)!")
  stop(spec_txt)}
  ...
  kegg_meta_df <- dplyr::filter(kegg_meta,tolower(kegg_meta$Description) !="metabolic pathways")
  ...
  #筛选出含有"METABOL"字段的行
  kegg_signal <- dplyr::filter(kegg_signal@result,grepl("SIGNAL",upper))
